...
function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled
      ? 'bg-dark-bg/95 backdrop-blur-md border-b border-white/10 shadow-lg border-b-2 border-white'
      : 'bg-dark-bg/90 backdrop-blur-sm'
      } overflow-x-hidden`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 py-3 sm:py-4 overflow-hidden">
        {/* Logo */}
        ...
        {/* Desktop Nav */}
        <DesktopNav navItems={navItems} handleScrollToSection={handleScrollToSection} />

        {/* Mobile Toggle */}
        <button
          className="lg:hidden text-white p-2.5 rounded-lg hover:bg-white/10 transition-all duration-200 focus:outline-none"
...
function DesktopNav({ navItems, handleScrollToSection }) {
  return (
...
function MobileNav({ isOpen, navItems, handleScrollToSection, closeMenu }) {
  return (
    <>
      <div className={`lg:hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-screen opacity-100 visible' : 'max-h-0 opacity-0 invisible'}`}>
      ...
      {isOpen && (
        <div className="lg:hidden fixed inset-0 bg-black/20 backdrop-blur-sm z-40" onClick={closeMenu}></div>
      )}
    </>
  );
}
